...
- If the monitor keeps running but its usage fetches fail, the cached value picks up a trailing `?` after 20 minutes and stops being published after an hour, so the gauges never freeze at plausible-looking numbers. A rate-limited poll counts as contact, so the deliberate 429 backoff doesn't age rows out.
